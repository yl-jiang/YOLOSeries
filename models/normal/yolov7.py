...
class YOLOV7Baseline(nn.Module):
    ...
    def __init__(self, num_anchor=3, num_class=80, in_channel=3) -> None:
        super(YOLOV7Baseline, self).__init__()

        self.backbone = BaselineBackbone(in_channel=in_channel)
        self.head = BaselineHead()
        self.detect = BaselineDetect(num_anchors=num_anchor, in_channels=[256, 512, 1024], num_classes=num_class)
    ...
    def _init_bias(self):
        ...
        detect_layer = [self.detect.detect_s,
                        self.detect.detect_m,
                        self.detect.detect_l]

        input_img_shape = 128
        stage_outputs = self(torch.zeros(1, 3, input_img_shape, input_img_shape).float())
        strides = torch.tensor([input_img_shape / x.size(2) for x in stage_outputs.values()])
