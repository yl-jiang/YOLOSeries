...
if __name__ == "__main__":
    ...
    coco = COCOGenerator("/Dataset/COCO2017/", "train2017", False)
    for i in tqdm(range(len(coco)), total=len(coco)):
        ...
        img_dst = Path("/Dataset/COCO2017/image/") / f"{img_id}.jpg"
        shutil.copy(str(img_src), str(img_dst))
        with open("/Dataset/COCO2017/label/" + f"{img_id}.txt", 'w') as f:
            for c, b in zip(cls_, box):
                f.write(f"{c} {b[0]} {b[1]} {b[2]} {b[3]}\n")

    with open("/Dataset/COCO2017/train/names.txt", 'w') as f:
