...
class RetinaNetEvaluator:
    ...
    def bbox_transform(self, anchors, regressions):
        ...
        """
        assert regressions.ndim == 3 and regressions.size(-1) == 4
    ...
    def do_inference(self, imgs):
        ...
        Args:
            imgs: (b, 3, h, w)
        Returns:
            preds_out: (b, N, num_class + 4) / [xmin, ymin, xmax, ymax, cls1, cls2, cls3, ...]
        """
        ...
        pred_box = self.bbox_transform(anchors, pred_box)  # (b, N, 4)
        pred_box = self.bbox_clip(imgs, pred_box)  # (b, N, 4)
        # preds_out: (b, N, 4) & (b, N, num_class) -> (b, N, num_class+4)
        preds_out = torch.cat((pred_box, pred_cls), dim=2)
        return preds_out
    ...
    def do_wfb(self, preds_out):
        ...
        Args:
            preds_out: [(batch_size, X, 84), (batch_size, Y, 84), (batch_size, Z, 84)] / [ xmin, ymin, xmax, ymax, cls1, cls2, cls3, ...]
        Returns:
        ...
        for i, preds in enumerate(preds_out):  # each model
            ...
            for b in range(batchsize):  # each image
                pred_cls = preds[b, :, 4:]  # (N, num_class)
                pred_box = preds[b, :, :4]  # (N, 4)
    ...
    def test_time_augmentation(self, inputs):  # just for inference not training time
        ...
        Args:
            inputs: (bn, 3, h, w)
        Returns:
            preds: (bs, M+N+P, 84) / [xmin, ymin, xmax, ymax, cls1, cls2, cls3, ...]
        """
        ...
        for s, f in zip(scale_facotr, flip_axis):
            ...
            # (bs, M, 84) / [xmin, ymin, xmax, ymax, cls1, cls2, cls3, ...]
            preds = self.do_inference(img)
            preds[..., :4] /= s
            if f == 2:  # flip axis y
                tmp_pred_ymin = preds[..., 1].clone()
                tmp_pred_ymax = preds[..., 3].clone()
                preds[..., 1] = img_h - tmp_pred_ymax
                preds[..., 3] = img_h - tmp_pred_ymin
                del tmp_pred_ymin, tmp_pred_ymax
            if f == 3:  # flip axis x
                tmp_pred_xmin = preds[..., 0].clone()
                tmp_pred_xmax = preds[..., 2].clone()
                preds[..., 0] = img_w - tmp_pred_xmax
                preds[..., 2] = img_w - tmp_pred_xmin
                del tmp_pred_xmin, tmp_pred_xmax
            # [(bs, M, 84), (bs, N, 84), (bs, P, 84)]
            tta_preds.append(preds)
        # (bs, M+N+P, 84) / [xmin, ymin, xmax, ymax, cls1, cls2, cls3, ...]
    ...
    def numba_nms(self, preds_out):
        ...
        do NMS with numba
        Args:
            preds_out: (bs, M+N+P, 84) / [xmin, ymin, xmax, ymax, cls1, cls2, cls3, ...]
        Returns:
