...
class Training:
    ...
    def __init__(self, hyp):
        ...
        self.validate = Evaluate(self.ema_model.ema if self.hyp['do_ema'] else self.model, self.hyp)
                ...
                with tqdm(total=len(self.traindataloader), file=sys.stdout) as tbar:
                    for i, x in enumerate(self.traindataloader):
                        ...
                        if cur_steps % int(self.hyp.get('show_tbar_every', 3)) == 0:
                            self.show_tbar(tbar, epoch+1, i, batchsz, start_t, is_best, tot_loss, iou_loss, cof_loss, cls_loss, l1_reg_loss, tar_nums, inp_h, map50, map, epoch_period)
